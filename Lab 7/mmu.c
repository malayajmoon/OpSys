...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    int block_end = blocknode->blk->end;

    if(!blocknode){
      print("Error: Memory Allocation <blocksize> blocks\n", blocksize);
    }
    ...
    if (blocknode->blk->end != block_end){
